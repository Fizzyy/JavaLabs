function setHeaderToAJAX(x, y, z, isAsync = true, isCombined = false) {
    ...
    xmlHttpRequest.open("POST", "http://localhost:8080/AS_ydsto/Sss_Header", isAsync);
...
function setHeaderGetXml(n, isAsync = true, isCombined = false) {
    ...
    xmlHttpRequest.open("POST", "http://localhost:8080/AS_ydsto/Sss_XML", isAsync);
...
function setHeaderGetJson(n, isAsync = true, isCombined = false) {
    let xmlHttpRequest = new XMLHttpRequest();
    xmlHttpRequest.open("POST", "http://localhost:8080/AS_ydsto/Sss_JSON", isAsync);
    xmlHttpRequest.setRequestHeader("XRand_n", n.value);
    xmlHttpRequest.setRequestHeader("task", isCombined ? "4" : "1");
...
function callAllFuncs(x, y, z, n, isAsync = true) {
...
function sendAjaxJQuery(n) {
    ...
    $.ajax({
        url: 'http://localhost:8080/AS_ydsto/Sss_JSON',
        type: 'POST',
        dataType: 'json',
        headers: {
            'XRand_n': n.value,
